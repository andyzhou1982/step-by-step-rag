...
Day 8: Added Wiki knowledge compilation configuration
Day 8： 添加了 Wiki 知识编译配置
"""
...
class Settings:
    ...
    def __init__(self):
        ...
        # Wiki Knowledge Compilation Configuration (Day 8)
        # Wiki 知识编译配置（Day 8）
        self.wiki_enabled: bool = os.getenv("WIKI_ENABLED", "true").lower() == "true"
        self.wiki_max_concepts_per_doc: int = int(os.getenv("WIKI_MAX_CONCEPTS_PER_DOC", "10"))
        self.wiki_max_pages: int = int(os.getenv("WIKI_MAX_PAGES", "50"))
        self.wiki_search_top_k: int = int(os.getenv("WIKI_SEARCH_TOP_K", "5"))
        self.wiki_auto_link: bool = os.getenv("WIKI_AUTO_LINK", "true").lower() == "true"
        self.wiki_link_min_confidence: float = float(os.getenv("WIKI_LINK_MIN_CONFIDENCE", "0.1"))
