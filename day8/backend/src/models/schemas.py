...
class WikiPageInfo(BaseModel):
    """
    Wiki page summary information
    Wiki 页面摘要信息

    Day 8: New model for Wiki page listing
    Day 8： Wiki 页面列表的新模型
    """
    id: str
    title: str
...
class WikiPageDetail(BaseModel):
    ...
    Day 8: New model for Wiki page detail view
    Day 8： Wiki 页面详情视图的新模型
    """
    id: str
    title: str
    content: str
...
class WikiPageListResponse(BaseModel):
    ...
    Day 8: New model for Wiki page list
    Day 8： Wiki 页面列表的新模型
    """
...
class WikiGenerateRequest(BaseModel):
    """
    Request to generate Wiki pages from documents
    从文档生成 Wiki 页面的请求

    Day 8: New model for Wiki generation
    Day 8： Wiki 生成的新模型
    """
    # Optional: specific document IDs to process
    # 可选：要处理的特定文档 ID
    document_ids: Optional[List[str]] = None
    # Maximum number of concepts to extract per document
    # 每个文档提取的最大概念数
    max_concepts_per_doc: int = 10
    # Maximum number of Wiki pages to generate
    # 最大生成的 Wiki 页面数
    max_pages: int = 50
...
class WikiGenerateResponse(BaseModel):
    """
    Response after generating Wiki pages
    生成 Wiki 页面后的响应

    Day 8: New model for Wiki generation response
    Day 8： Wiki 生成响应的新模型
    """
    pages_generated: int
...
class WikiSearchRequest(BaseModel):
    ...
    Day 8: New model for Wiki search
    Day 8： Wiki 搜索的新模型
    """
    query: str
    top_k: int = 5
...
class WikiSearchResult(BaseModel):
    """
    A single Wiki page search result
    单个 Wiki 页面搜索结果

    Day 8: New model for Wiki search result
    Day 8： Wiki 搜索结果的新模型
    """
    page: WikiPageInfo
    score: float
