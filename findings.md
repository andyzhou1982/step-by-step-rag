...
## Phase 2 Research: 三种 RAG 架构融合
/*
  Phase 2 研究：传统 RAG + LLM Wiki + GraphRAG 三架构融合
  Updated: 2026-05-14
*/

### 三种架构核心对比
| 维度 | 传统 RAG | LLM Wiki | GraphRAG |
|------|----------|----------|----------|
| 核心机制 | 向量检索 + LLM 生成 | 预编译/总结成 Wiki | 图结构构建 + 图检索 + LLM |
| 知识表达 | chunks/embeddings | 抽象层结构化语义 | 图结构知识网络 |
| 查询依赖 | 实时检索原文片段 | 语义总结 + 查询时匹配 | 多跳推理、关系检索 |
| 源信息准确性 | ⭐⭐⭐⭐ | ⭐⭐⭐ | ⭐⭐⭐⭐ |
| 多跳推理 | ⭐⭐ | ⭐⭐⭐⭐ | ⭐⭐⭐⭐ |
| 构建成本 | ⭐⭐⭐⭐(低) | ⭐⭐(高) | ⭐⭐(高) |
| 更新成本 | ⭐⭐⭐⭐(低) | ⭐⭐(高) | ⭐⭐-⭐⭐⭐ |
...
用户提问
  → LLM 分析问题类型:
    - 事实查询/细节查找 → 传统 RAG (高保真原文)
    - 概念解释/总结/综述 → LLM Wiki (高层语义)
    - 关系查询/因果推理/X和Y的关系 → GraphRAG (图遍历)
    - 复杂综合 → 多路并行 + 结果融合
```

### LLM Wiki 关键技术点
1. **Wiki 页面生成**: LLM 读取文档 → 提取核心概念 → 生成结构化 Wiki 页面
2. **概念提取**: 命名实体识别 + 概念聚类 + 去重
3. **一致性检查**: Wiki 页面 vs 原文比对，自动标注偏差
4. **纠偏策略**: 周期性批量更新 + 人工审核队列（非实时，控制成本）
5. **交叉引用**: Wiki 页面间自动建立概念链接

### GraphRAG 关键技术点
1. **实体抽取**: LLM 从文本中抽取实体（人物、组织、概念、事件等）
2. **关系抽取**: LLM 识别实体间关系（属于、导致、相关、引用等）
3. **实体消歧**: 同名实体统一、别名处理、实体合并
4. **社区检测**: 图聚类算法 → 发现实体社区 → 生成社区摘要
5. **多跳推理**: 沿关系路径 2-3 跳推理，将路径转化为 LLM 上下文
6. **图存储**: PostgreSQL 邻接表（nodes 表 + edges 表），保持技术栈一致

### 混合架构设计
```
Layer 1: 向量 RAG (传统) → 保证 fidelity（高保真）
Layer 2: 图层增强 (GraphRAG) → 关系理解、多跳推理
Layer 3: Wiki 层 (LLM Wiki) → 长期语义记忆、高层逻辑
Router:  LLM 智能路由 → 根据问题特征选择最佳策略
```
